...
export const UpcomingMeetingsWidget: React.FC = () => {
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm h-full flex flex-col">
      ...
      <div className="flex flex-col gap-1 relative flex-1 min-h-0 overflow-y-auto custom-scrollbar pr-2">
        <div className="absolute left-[15px] top-4 bottom-4 w-px bg-slate-100" />
        
        {meetings.length === 0 ? (
          <div className="text-xs text-slate-400 py-4 text-center z-10 bg-white">No upcoming meetings</div>
        ) : (
          meetings.slice(0, 3).map((meeting: any, idx: number) => (
            <div key={meeting.id} className="flex gap-4 relative py-2 group cursor-pointer z-10">
