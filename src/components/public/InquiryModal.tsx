    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      >
        ...
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-white/90 backdrop-blur-xl sticky top-0 z-10">
           ...
           <button 
             onClick={onClose} 
             className="p-2.5 bg-slate-50 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-full transition-all active:scale-95 border border-slate-200 shadow-sm"
           >
             <X className="w-4 h-4" />
           </button>
        </div>
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="group">
                    ...
                    <input type="text" required placeholder="John Doe" className={inputClasses} onFocus={() => setFocusedField('name')} onBlur={() => setFocusedField(null)} />
                  </div>
                  <div className="group">
                    <label className={`${labelClasses} ${focusedField === 'company' ? 'text-kairo-blue' : ''}`}>Business / Company</label>
                    <input type="text" required placeholder="Acme Corp" className={inputClasses} onFocus={() => setFocusedField('company')} onBlur={() => setFocusedField(null)} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="group">
                    ...
                    <input type="email" required placeholder="john@example.com" className={inputClasses} onFocus={() => setFocusedField('email')} onBlur={() => setFocusedField(null)} />
                  </div>
                  <div className="group">
                    <label className={`${labelClasses} ${focusedField === 'phone' ? 'text-kairo-blue' : ''}`}>WhatsApp Number</label>
                    <input type="tel" required placeholder="+1 (555) 000-0000" className={inputClasses} onFocus={() => setFocusedField('phone')} onBlur={() => setFocusedField(null)} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="group">
                    ...
                    <select className={inputClasses} required defaultValue="" onFocus={() => setFocusedField('type')} onBlur={() => setFocusedField(null)}>
                      <option value="" disabled>Select project type...</option>
                      <option value="dashboard">Custom SaaS Dashboard</option>
                      <option value="internal_os">Internal Operations System</option>
                      <option value="ai_agent">AI Automation Workspace</option>
                  ...
                  <div className="group">
                    ...
                    <select className={inputClasses} required defaultValue="" onFocus={() => setFocusedField('budget')} onBlur={() => setFocusedField(null)}>
                ...
                <div className="group">
                  ...
                  <textarea required placeholder="What are the primary operational objectives of this build?" rows={3} className={`${inputClasses} resize-none`} onFocus={() => setFocusedField('goals')} onBlur={() => setFocusedField(null)} />
                </div>

                <div className="group">
                  <label className={`${labelClasses} ${focusedField === 'notes' ? 'text-kairo-blue' : ''}`}>Timeline & Additional Notes</label>
                  <textarea placeholder="Any strict deadlines or specific infrastructure requirements?" rows={3} className={`${inputClasses} resize-none`} onFocus={() => setFocusedField('notes')} onBlur={() => setFocusedField(null)} />
                </div>
